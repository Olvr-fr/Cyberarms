...
namespace Cyberarms.IntrusionDetection {
    internal class FirewallPolicyManager {
        private dynamic firewallPolicyManager;
        ...
        internal void CleanUpRules() {
            foreach (dynamic rule in FindRules(Globals.CYBERARMS_WINDOWS_IDS_RULE_NAME)) {
                //rule.RemoteAddresses = "";
                firewallPolicyManager.Rules.Remove(rule.Name);
            }
        }

        internal dynamic GetRule(string name) {
            foreach (dynamic rule in firewallPolicyManager.Rules) {
                if (rule.Name == name) return rule;
            }
            return null!;
        }

        internal List<dynamic> FindRules(string name) {
